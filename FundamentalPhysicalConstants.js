...
text.split('\n').slice(2).forEach(line => {
  if (!line.trim()) return;
  const qty = line.slice(0, 60).trim();
  const val = line.slice(60, 85).trim().replace(/ /g, '');
  const unc = line.slice(85, 110).trim().replace(/ /g, '');
  const unit = line.slice(110).trim();
  if (qty) constants[qty] = { value: val, uncertainty: unc, unit };
});
// Access: constants['Avogadro constant'].value
